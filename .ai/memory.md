...
## 2026-04-13 - 文字颜色与最近使用调色盘（增量）

### 文字颜色
- `Bubble` 新增 `textColor` 字段
- 画布编辑区、页面缩略预览、导出结果中的文字颜色都改为使用 `bubble.textColor`
- 旧项目加载时若缺少该字段，会自动回退到默认深色文字 `#0f172a`

### 最近使用调色盘
- `InspectorPanel` 的文字属性区新增：
  - 字体颜色选择器
  - 最近使用颜色调色盘
- 调色盘最多保留最近使用过的 5 种文字颜色，可一键复用
- 最近颜色记录保存在浏览器 `localStorage`：
  - key: `openkoma-recent-text-colors`
- 加载项目时会自动把项目里已有的文字颜色合并进最近调色盘，方便继续复用

### 存档
- 项目文件版本号提升到 `6`
- 最近使用调色盘属于编辑器本地偏好，不写入项目文件；真正写入项目的是每个文字框自己的 `textColor`

## 2026-04-07 - 回归纯本地编辑器，移除外部服务能力

### 当前定位
- OpenKoma 现在是纯本地漫画分镜编辑器，只保留：
  - 本地图片导入
  - 非破坏手动裁剪
  - 多页面编辑
  - 本地保存 / 加载 / 导出
- 不再提供任何外部图像服务接入能力，也不再需要服务配置或身份认证

### 已移除内容
- 删除外部服务配置状态、工具栏入口和健康检查按钮
- 删除生图、去背景、超分相关前端调用与数据结构
- 删除分镜上的 `prompt` / `negativePrompt` 字段
- 旧的 FastAPI 接口文档已废弃

### 本地资源约束
- 只允许导入本地图片作为分镜素材
- 保存项目时只会序列化目录资源、`blob:` 或 `data:` 图像引用，不再尝试抓取远端图片
- 旧项目里若残留外链图片引用，会被视为不可物化资源，保存时可能提示未能写入
